...
export default class DestroyCommunityItemMutation extends Relay.Mutation {
    // This method should return a GraphQL operation that represents
    // the mutation to be performed. This presumes that the server
    // implements a mutation type named ‘likeStory’.
    getMutation() {
        return Relay.QL`mutation { destroyCommunityItem }`;
    }
    ...
    getFatQuery() {
        return Relay.QL `
            fragment on DestroyCommunityItemPayload {
                viewer {
                    responseErrorsList
                }
            }
